    ...
    return (
        <div className={`${className} w-full mt-[80px] flex relative h-auto`}>
            ...
            <div className="container-testi bg-black bg-opacity-50 w-full flex flex-col items-center justify-center relative h-auto z-10">
                <h1 className="font-archivo text-2xl font-bold text-white mt-8 sm:text-3xl md:text-5xl">{getWcmsValue(TESTIMONI_TITLE)}</h1>
                <h2 className="font-archivo text-lg font-bold text-white mt-4 text-center sm:text-xl  md:text-3xl">{getWcmsValue(TESTIMONI_SUBTITLE)}</h2>
                <div className="w-[100px] h-1 bg-white mt-4"></div>
                <p className="text-justify text-white mt-4 font-alice text-md px-8 sm:px-[10%] sm:text-xl xl:px-[30%]">{getWcmsValue(TESTIMONI_DESC)}</p>
